...
| 2. Worker/DO integration     | `@cloudflare/vitest-pool-workers` in real workerd: real WebSockets end to end, plus `runInDurableObject` where a test needs to reach into storage                                  | Storage, alarms, the whole command surface, the privacy filter, the authorization matrix (derived from the command schema), and stream/snapshot convergence. **As built**: storage is isolated per FILE, not per test, so the WS flow tests simply live in their own file — the separate `--no-isolate` project the plan called for was never needed. There are no eviction helpers and therefore **no test yet proves hibernation survival**, which rule 2 makes load-bearing. Pool-workers is beta: **pin vitest+pool-workers+wrangler as a set** |
