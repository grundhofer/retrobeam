...
- **The same filter paces the presenting round**: `revealFor(phase, role, picker, anonymous)` in `packages/shared` returns what one viewer may see — own notes only before the reveal (the facilitator included), the whole board for a facilitator or any later phase, and during the round a member's own cards plus the authors the rotation has already staged. Every widening goes through one delta helper in the DO (`broadcastNewlyVisible`), which diffs the gate before and after and sends each recipient exactly what just became visible, so a widening can neither miss a note nor send one twice. Which authors have been shown is a **separate, append-only field on the picker** (`revealed`) rather than a derivation from `presented`/`current`: those go backwards on a skip, and the server has no un-reveal event — nothing the room has read may ever be taken back. A stack id is its anchor note's id, so a note whose anchor the viewer cannot see is delivered ungrouped and re-sent once the anchor appears; `symmetry.test.ts` is what holds all of this to the snapshot. Two things this
  rule makes load-bearing that are easy to miss: the schema version is stamped at
  board **creation**, because `migrate()` runs in the constructor before the board
  row exists and a board that never got stamped would take the back-compat step
  meant for older boards the first time it woke mid-round; and a command's ack
  shape is part of the filter — unstacking a card whose anchor the caller cannot
  see answers exactly like unstacking a loose one, or the following `note.updated`
  re-derives the bit the redaction withholds.
...
- All server-side: phase gates, vote budgets, authorship checks, write-phase redaction, presenter-scoped redaction during the presenting round, picker draws. The client is untrusted rendering. The export route is unauthenticated by design and therefore passes through the same reveal a viewer with no identity would get — otherwise a download would be a way around the round.
...
| 2. Worker/DO integration     | `@cloudflare/vitest-pool-workers` in real workerd: real WebSockets end to end, plus `runInDurableObject` where a test needs to reach into storage                                  | Storage, alarms, the whole command surface, the privacy filter, the authorization matrix (derived from the command schema), and stream/snapshot convergence. **As built**: storage is isolated per FILE, not per test, so the WS flow tests simply live in their own file — the separate `--no-isolate` project the plan called for was never needed. Hibernation survival IS covered: `evictAllDurableObjects()` tears the instance down while keeping storage and hibernating the sockets, and the test then drives the SAME socket and asserts identity, role, phase and notes all came back. Verified to fail when a single value is moved into an instance field. Pool-workers is beta: **pin vitest+pool-workers+wrangler as a set** — and note that the pool depends on its own exact wrangler, so the workspace devDep is not what parses `wrangler.jsonc` in tests. **This package name is a dead end: 0.22.0 is its last release; v1 ships as `@cloudflare/vitest-plugin` (codemod: `npx @cloudflare/codemods vitest:pool-workers-to-vitest-plugin`), so dependabot will simply stop offering updates.** Two behaviour changes that came in with 0.19-0.22 and are worth knowing before debugging a surprise: worker-disposal errors are now logged instead of failing a run (`NODE_DEBUG=vitest-pool-workers` to see them), which makes one class of teardown failure quiet; and the emulated Ratelimit binding keeps its counters in Durable Object storage instead of the JS heap, so they no longer reset after ~10s idle — `freshIp()` in `test/helpers.ts` is what keeps the suite from throttling itself, and it matters more now than it did |
